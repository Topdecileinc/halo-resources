...
- The campaign-specific middle (hero, offer cards, feature rows) is built fresh per email or from a `template_` file when no `section_*.html` exists for it yet. As those patterns stabilize, promote them to `section_*.html` files. **Fresh campaign HTML doesn't need a section/component class** — only files-as-sources get classes.
