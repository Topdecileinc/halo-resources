...
4. **Pick a starting point** — three cases, in order:
   ...
   - **No template and no layout given** → build a **minimal email** — the essentials only: header section → hero (when the brief gives a hero URL) → headline/subhead → body copy (the sales pitch) → any CTA and pricing the brief provides → footer section. Use the `sample_` emails only for tone and HTML structure, and assemble from `section_*.html` files (with `component_*.html` primitives inside them where needed). **Do not add a feature/value section, a spec or model-comparison grid, a reviews/social-proof block, or a membership section on your own** — those are opt-in and appear only when the brief's §7 names them. Having product facts on hand in the rule files does not mean you should showcase them. Structure/tone come from the samples; all visual values from the style guide; product facts come verbatim from the rule files; campaign specifics from the brief.
